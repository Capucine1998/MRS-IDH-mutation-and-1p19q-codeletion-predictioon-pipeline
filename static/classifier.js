...
async function runSecondClassifier(userFolder) {
    ...
    try {
        ...
        // 1p/19q SHAP images are generated by the second classifier, so we must
        // re-attempt SHAP plot loading after this step completes.
        const inferredUsedMegaDiff = typeof result.predictions_csv === 'string'
            ? result.predictions_csv.includes('mega_off_and_diff')
            : false;
        await displayShapPlots(userFolder, inferredUsedMegaDiff);
